...
const ChangeFormPage: React.FC = () => {
	...
	const handleQuestionChange = (index: number, newQuestion: string, sectionIndex: number, cardType: string, subQuestionIndex?: number) => {
		...
		if (cardType === "subQuestion") {
			...
			console.log(newSections, index, sectionIndex)
						...
						<Toolbar sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
							<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
								<Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
									...
									<Typography variant='h5' color="black">Изменение форма</Typography>
								</Box>
												...
												<Grid item xs={12}
													...
													<Box sx={{ mb: 3 }}>
														<Paper elevation={2} sx={{
															...
															{card.selectedComponent === 'Input' && <InputCopmponent idQuestion={card.idQuestion} disabled={true} quest={card.question} required={card.isRequired} cardFormPageType={'card'} changeForm={true} />}
															{card.selectedComponent === 'Textarea' && <TextareaComponent idQuestion={card.idQuestion} disabled={true} quest={card.question} required={card.isRequired} cardFormPageType={'card'} changeForm={true} />}
															{card.selectedComponent === 'Radio' && <RadioComponent idQuestion={card.idQuestion} disabled={true} answers={card.answer} quest={card.question} required={card.isRequired} cardFormPageType={'card'} changeForm={true} />}
															{card.selectedComponent === 'Checkbox' && <CheckboxesComponent idQuestion={card.idQuestion} disabled={true} answers={card.answer} quest={card.question} required={card.isRequired} addLogic={card.addLogic} GetLogic={card.Logic} changeCardsLogic={card.changeCardsLogic} cardFormPageType={'card'} changeForm={true} />}
															{card.selectedComponent === 'Slider' && <SliderComponent idQuestion={card.idQuestion} disabled={true} answers={card.answer} quest={card.question} required={card.isRequired} changeCardsLogic={card.changeCardsLogic} cardFormPageType={'card'} changeForm={true} />}
															{card.selectedComponent === 'Data' && <DataComponent idQuestion={card.idQuestion} disabled={true} quest={card.question} required={card.isRequired} cardFormPageType={'card'} changeForm={true} />}
															<Grid item xs={12}>
